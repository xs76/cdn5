...
function poki_loadbar(ctx, width, height, total, current, image) {
    ...
    var backgroundColor2 = getc("background_color2", "#FFFFFF");
    ...
    var loadingString = getc("loading_string", "Loading...");
    // background:
    ...
    if (typeof ctx.roundRect === 'function') {
        ctx.beginPath();
        ctx.roundRect(barLeft, barTop+4, barWidth, barHeight+4, 16);
        ctx.fill();
    }
    else {
    ...
    if (typeof ctx.roundRect === 'function') {
        ctx.beginPath();
        ctx.roundRect(barLeft, barTop, barWidth, barHeight, 16);
        ctx.fill();
    }
    else {
    ...
    if (typeof ctx.roundRect === 'function') {
        ctx.beginPath();
        ctx.roundRect(barInnerLeft, barInnerTop, barInnerWidth, barInnerHeight, 16);
        ctx.fill();
    }
    else {
    ...
    if (typeof ctx.roundRect === 'function') {
        ctx.beginPath();
        ctx.roundRect(barInnerLeft-1, barInnerTop, barLoadedWidth, barInnerHeight, 16);
        ctx.fill();
    }
    else {
        ctx.fillRect(barInnerLeft-1, barInnerTop, barLoadedWidth, barInnerHeight);
    }
    // loading text
    /*ctx.font = "20px Verdana";
    ctx.textAlign = "center";
    ctx.fillText(loadingString, barLeft+barWidth/2, barTop-24);*/
}
